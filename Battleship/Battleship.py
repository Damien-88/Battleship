class Player:
    ...
    def attack(self, new_target):
        ...
        self.target = new_target.capitalize()
        available = False
        index = 0

        for row in range(0, len(self.available_targets)):
            for space in range(0, len(self.available_targets[row])):
                if self.available_targets[row][space] == self.target:
                    index = row
                    available = True
